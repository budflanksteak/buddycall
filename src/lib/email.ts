...
export async function sendPasswordResetEmail(email: string, name: string, resetUrl: string) {
  await transporter.sendMail({
    ...
    to: email,
    subject: 'Reset your Neurorad Call Autopilot password',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #1e40af;">Neurorad Call Autopilot</h2>
        <p>Hi ${name},</p>
...
export async function sendVerificationEmail(email: string, name: string, verifyUrl: string) {
  await transporter.sendMail({
    from: process.env.EMAIL_FROM,
    to: email,
    subject: 'Verify your Neurorad Call Autopilot account',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #1e40af;">Welcome to Neurorad Call Autopilot!</h2>
        <p>Hi ${name},</p>
